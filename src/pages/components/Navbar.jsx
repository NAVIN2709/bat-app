  ...
  return (
    <nav className="sticky top-0 z-50 backdrop-blur-xl bg-white/80 border-b border-green-100 shadow-[0_2px_10px_rgba(0,0,0,0.05)] w-full overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-5 py-3 flex items-center justify-between">
        {/* LOGO + TEXT */}
        <div className="flex items-center gap-3 cursor-pointer group" onClick={handleLogo}>
          <img
            src={Logo}
            alt="Logo"
        ...
        {/* Mobile Menu Button */}
        <button className="sm:hidden p-2 rounded-md text-green-700" onClick={() => setOpen(!open)}>
      ...
      {open && (
        <div className="sm:hidden bg-white px-5 pb-5 pt-2 shadow-md animate-slideDown overflow-x-hidden flex flex-col gap-2">
          <button className="w-full bg-green-600 text-white px-4 py-2 rounded-lg text-lg font-semibold transition duration-300 hover:bg-white hover:text-green-600 hover:border hover:border-green-600 hover:scale-105">
            Get Membership
          </button>
